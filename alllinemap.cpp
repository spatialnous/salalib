...
void AllLineMap::generate(Communicator *comm, std::vector<Line> &lines,
  ...
  if (comm) {
    ...
    comm->CommPostMessage(Communicator::NUM_RECORDS,
                          m_polygons.m_vertex_possibles.size());
  }

  std::set<AxialVertex> openvertices;
  openvertices.insert(vertex);
  while (!openvertices.empty()) {
    m_polygons.makeAxialLines(openvertices, axiallines, preaxialdata,
                              m_poly_connections, m_radial_lines);
    count++;
    //
    if (comm) {
      if (qtimer(atime, 500)) {
  ...
  for (size_t j = 0; j < axiallines.size(); j++) {
    for (size_t k = axiallines.size() - 1; k > j; k--) {
      ...
      if (approxeq(axiallines[j].start(), axiallines[k].start(),
                   maxdim * TOLERANCE_B) &&
          approxeq(axiallines[j].end(), axiallines[k].end(),
                   maxdim * TOLERANCE_B)) {
  ...
  for (size_t k = 0; k < axiallines.size(); k++) {
    axiallines[k].crop(
        region); // <- should be cropped anyway, but causing an error
...
AllLineMap::extractFewestLineMaps(Communicator *comm) {
  ...
  for (auto &radial_line : m_radial_lines) {
    radialdivisions.insert(
        std::make_pair((RadialKey)radial_line, std::set<int>()));
  }
  ...
  makeDivisions(m_poly_connections, m_radial_lines, radialdivisions,
                ax_radial_cuts, comm);
  ...
  if (iter != m_radial_lines.end()) {
    ...
    for (; iter != m_radial_lines.end();) {
      if (iter->vertex == prevIter->vertex && iter->ang != prevIter->ang) {
        radialsegs.insert(
            std::make_pair((RadialKey)(*iter), (RadialSegment)(*prevIter)));
      }
      ++iter;
      ++prevIter;
    }
      ...
      for (size_t j = 1; j < axIter->second.size(); ++j) {
        ...
        if (rk_start.vertex == rk_end.vertex) {
          ...
          if (radialSegIter != radialsegs.end() &&
            ...
            axSeg->second.insert(
                std::distance(radialsegs.begin(), radialSegIter));
          }
        }
        ++axRadCutIter;
        ++axRadCutIterPrev;
  ...
  minimiser.removeSubsets(ax_seg_cuts, radialsegs, radialdivisions,
                          m_radial_lines, keyvertexconns, keyvertexcounts);
  ...
  minimiser.fewestLongest(ax_seg_cuts, radialsegs, radialdivisions,
                          m_radial_lines, keyvertexconns, keyvertexcounts);
  ...
  for (int k = 0; k < int(getAllShapes().size()); k++) {
    if (!minimiser.removed(k)) {
      lines_m.push_back(
          depthmapX::getMapAtIndex(getAllShapes(), k)->second.getLine());
    }
  }

  std::unique_ptr<ShapeGraph> fewestlinemap_subsets(
      new ShapeGraph("Fewest-Line Map (Subsets)", ShapeMap::AXIALMAP));
  ...
  std::unique_ptr<ShapeGraph> fewestlinemap_minimal(
  ...
  return std::make_tuple(std::move(fewestlinemap_subsets),
                         std::move(fewestlinemap_minimal));
}

void AllLineMap::makeDivisions(
    const std::vector<PolyConnector> &polyconnections,
  ...
  for (size_t i = 0; i < polyconnections.size(); i++) {
    ...
    for (size_t j = 0; j < pixels.size(); j++) {
      ...
      for (const ShapeRef &shape : shapes) {
        ...
        //
        if (intersect_region(line, polyconnections[i].line,
          ...
          case 0:
            break;
          case 2: {
            size_t index = depthmapX::findIndexFromKey(axialdividers,
                                                       (int)shape.m_shape_ref);
