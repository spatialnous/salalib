...
float AttributeRowImpl::getNormalisedValue(size_t index) const {
  ...
  return m_data[index] < 0 ? -1.0f
                           : float((m_data[index] - colStats.min) /
                                   (colStats.max - colStats.min));
}

AttributeRow &AttributeRowImpl::setValue(const std::string &column,
                                         float value) {
...
AttributeRow &AttributeRowImpl::incrValue(const std::string &colName,
                                          float value) {
...
AttributeRow &AttributeTable::addRow(const AttributeKey &key) {
  ...
  auto res = m_rows.insert(std::make_pair(
      key, std::unique_ptr<AttributeRowImpl>(new AttributeRowImpl(*this))));
...
void AttributeTable::renameColumn(const std::string &oldName,
...
void AttributeTable::setDisplayParamsForAllAttributes(
...
void AttributeTable::write(std::ostream &stream,
...
size_t AttributeTable::getColumnSortedIndex(size_t index) const {
  ...
  return std::distance(m_columnMapping.begin(),
                       m_columnMapping.find(getColumnName(index)));
}
...
size_t AttributeTable::addColumnInternal(const std::string &name,
                                         const std::string &formula) {
