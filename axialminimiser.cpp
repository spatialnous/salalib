...
AxialMinimiser::AxialMinimiser(const AllLineMap &alllinemap,
                               int no_of_axsegcuts, int no_of_radialsegs) {
...
void AxialMinimiser::removeSubsets(
  ...
  for (const auto &axSegCut : axsegcuts) {
    ...
    length = depthmapX::getMapAtIndex(m_alllinemap->m_shapes, y)
                 ->second.getLine()
                 .length();
    ...
    for (size_t i = 0; i < m_axialconns.size(); i++) {
      ...
      for (size_t j = 0; j < axa.m_connections.size(); j++) {
        ...
        if (axa.m_connections.size() <= axb.m_connections.size()) {
          ...
          for (size_t axai = 0, axbi = 0; axai < axa.m_connections.size() &&
                                          axbi < axb.m_connections.size();
               axai++, axbi++) {
            ...
            while (axbi < axb.m_connections.size() &&
                   (axb.m_connections[axbi] == ii ||
                    axa.m_connections[axai] > axb.m_connections[axbi])) {
              axbi++;
            }
      ...
      if (subset) {
        ...
        auto &axSegCut =
            depthmapX::getMapAtIndex(axsegcuts, removeindex)->second;
        ...
        if (!m_vital[removeindex]) {
          ...
          for (auto affectedconnection : affectedconnections) {
            if (!m_removed[affectedconnection]) {
              auto &connections =
                  m_axialconns[affectedconnection].m_connections;
...
void AxialMinimiser::fewestLongest(
    std::map<int, std::set<int>> &axsegcuts,
    std::map<RadialKey, RadialSegment> &radialsegs,
    std::map<RadialKey, std::set<int>> &rlds,
    std::vector<RadialLine> &radial_lines,
  ...
  for (size_t y = 0; y < m_axialconns.size(); y++) {
    if (!m_removed[y] && !m_vital[y]) {
      ...
      m_vps[livecount].value2 =
          (float)depthmapX::getMapAtIndex(m_alllinemap->m_shapes, y)
              ->second.getLine()
              .length();
      livecount++;
    }
  ...
  for (int i = 0; i < livecount; i++) {
    ...
    if (!presumedvital) {
      ...
      for (auto affectedconnection : affectedconnections) {
        if (!m_removed[affectedconnection]) {
          auto &connections =
              m_axialconns[size_t(affectedconnection)].m_connections;
          if (connections.size() <=
              2) { // <- note number of connections includes
    ...
    if (m_radialsegcounts[cut] <= 1) {
      ...
      if (iterKey == radial_lines.end()) {
        throw depthmapX::RuntimeException(
            "Radial key not found in radial lines");
      }
      ...
      for (int diva : divisorsa) {
        ...
        for (int divb : divisorsb) {
          ...
          if (std::find(connections.begin(), connections.end(), divb) !=
            ...
            Point2f p =
                intersection_point(axiallines[diva].getLine(),
                                   axiallines[divb].getLine(), TOLERANCE_A);
            if (p.insegment(rlinea.keyvertex, rlinea.openspace,
                            rlineb.openspace, TOLERANCE_A)) {
              nonvitalseg = true;
            }
          }
        }
