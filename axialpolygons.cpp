...
AxialVertex AxialPolygons::makeVertex(const AxialVertexKey &vertexkey,
                                      const Point2f &openspace) {
  auto vertPossIter =
      depthmapX::getMapAtIndex(m_vertex_possibles, vertexkey.m_ref_key);
  ...
  for (size_t i = 0; i < pointlist.size(); ++i) {
    anglemap.insert(
        std::make_pair(angle(openspace, av.m_point, pointlist[i]), i));
  }
  ...
  if (fabs(oa) < TOLERANCE_A || fabs(ob) < TOLERANCE_A ||
      fabs(ab) < TOLERANCE_A) {
...
void AxialPolygons::makeVertexPossibles(
  ...
  for (i = 0; i < lines.size(); i++) {
    if (found(0, i) == -1) {
      ...
      m_vertex_possibles.insert(
          std::make_pair(pointlookup.back(), std::vector<Point2f>()));
      m_vertex_polys.push_back(
      ...
      for (auto &segconn : connectionset[i].m_back_segconns) {
        ...
        found(static_cast<size_t>(forwback),
              static_cast<size_t>(segconn.first.ref)) = found(0, i);
      }
    }
    if (found(1, i) == -1) {
      ...
      for (auto &segconn : connectionset[i].m_forward_segconns) {
        int forwback = (segconn.first.dir == 1) ? 0 : 1;
        found(static_cast<size_t>(forwback),
              static_cast<size_t>(segconn.first.ref)) = found(1, i);
      }
    }
  }
  ...
  for (i = 0; i < lines.size(); i++) {
    ...
    if (index0 == m_vertex_possibles.end() ||
        index1 == m_vertex_possibles.end()) {
      // TODO: (CS) What are these integers being thrown?!
      throw 2;
    }
  ...
  for (auto &possible : m_vertex_possibles) {
    ...
    possible.second.erase(
        unique(possible.second.begin(), possible.second.end()),
        possible.second.end());
  }
    ...
    if (m_vertex_polys[i] == -1) {
      ...
      while (addlist.size()) {
        ...
        std::vector<Point2f> &connections =
            depthmapX::getMapAtIndex(m_vertex_possibles, addlist.back())
                ->second;
        addlist.pop_back();
        for (size_t j = 0; j < connections.size(); j++) {
          int index =
              depthmapX::findIndexFromKey(m_vertex_possibles, connections[j]);
          if (index == -1) {
            throw 3;
          }
...
void AxialPolygons::makePixelPolys() {
  ...
  for (const auto &vertPoss : m_vertex_possibles) {
    ...
    m_pixel_polys(static_cast<size_t>(pix.y), static_cast<size_t>(pix.x))
        .push_back(j);
  }
}
...
AxialVertexKey AxialPolygons::seedVertex(const Point2f &seed) {
  ...
  while (!foundvertex) {
    for (int vertexref : m_pixel_polys(static_cast<size_t>(seedref.y),
                                       static_cast<size_t>(seedref.x))) {
      const Point2f &trialpoint =
          depthmapX::getMapAtIndex(m_vertex_possibles, vertexref)->first;
...
void AxialPolygons::makeAxialLines(std::set<AxialVertex> &openvertices,
  ...
  for (const auto &vertPoss : m_vertex_possibles) {
    ...
    if (vertex.m_convex) {
      ...
      if (det(p, vertex.m_a) * det(p, vertex.m_b) < 0) {
        possible = true;
      } else if (det(p, vertex.m_a) < TOLERANCE_A &&
                 det(p, vertex.m_b) < TOLERANCE_A) {
        stubpossible = true;
      }
    }
    if (possible || stubpossible) {
      ...
      if (!intersect_exclude(line)) {
        ...
        if (next_vertex.m_initialised &&
            std::find(m_handled_list.begin(), m_handled_list.end(),
                      next_vertex) == m_handled_list.end()) {
          openvertices.insert(
              next_vertex); // <- note, add ignores duplicate adds (each
          ...
          if (!vertex.m_convex && possible) {
            Line ext(line.t_end(),
                     line.t_end() + (line.t_end() - line.t_start()));
            ext.ray(1, m_region);
            cutLine(ext, 1);
          ...
          if (m_vertex_polys[vertex.m_ref_key] !=
              m_vertex_polys[next_vertex.m_ref_key]) { // must be on separate
                                                       // polygons
            // radial line(s) (for new point)
            RadialLine radialshort(next_vertex, shortline_segend,
                                   vertex.m_point, next_vertex.m_point,
                                   next_vertex.m_point + next_vertex.m_b);
          ...
          if (!next_vertex.m_convex && next_vertex.m_axial) {
            Line ext(line.t_start() - (line.t_end() - line.t_start()),
                     line.t_start());
            ext.ray(0, m_region);
            cutLine(ext, 0);
          ...
          if (m_vertex_polys[vertex.m_ref_key] !=
            ...
            RadialLine radialshort(vertex, shortline_segend,
                                   next_vertex.m_point, vertex.m_point,
                                   vertex.m_point + vertex.m_b);
            poly_connections.push_back(
                PolyConnector(shortline, (RadialKey)radialshort));
            radial_lines.push_back(radialshort);
            if (!next_vertex.m_convex && next_vertex.m_axial) {
  ...
  radial_lines.erase(std::unique(radial_lines.begin(), radial_lines.end()),
                     radial_lines.end());
}
  ...
  for (const auto &vertPoss : m_vertex_possibles) {
    ...
    for (size_t j = 0; j < vertPoss.second.size(); j++) {
      ...
      while (curr != key) {
        ...
        for (size_t k = 0; k < vertPossIter->second.size(); k++) {
          ...
          if (last != next) {
            ...
            if (thisangle < minangle) {
              ...
              if (m_vertex_possibles.find(vertPossIter->second.at(k))
                      ->second.size() > 1) {
                minangle = thisangle;
                winner = k;
              }
        ...
        handled_list[std::distance(m_vertex_possibles.begin(), vertPossIter)]
            .push_back(winner);
        last = curr;
