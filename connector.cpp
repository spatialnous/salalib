...
int Connector::getConnectedRef(int cursor, int mode) const {
  ...
  if (cursor != -1) {
    ...
    case SEG_CONN_ALL:
      ...
      } else if (size_t(cursor) - m_back_segconns.size() <
                 m_forward_segconns.size()) {
...
int Connector::direction(int cursor, int mode) const {
  ...
  if (cursor != -1) {
    ...
    case SEG_CONN_ALL:
      if (cursor < (int)m_back_segconns.size()) {
        direction =
            depthmapX::getMapAtIndex(m_back_segconns, cursor)->first.dir;
      } else if (size_t(cursor) - m_back_segconns.size() <
                 m_forward_segconns.size()) {
        direction =
            depthmapX::getMapAtIndex(m_forward_segconns,
                                     cursor - int(m_back_segconns.size()))
                ->first.dir;
      }
      break;
    case SEG_CONN_FW:
      direction =
          depthmapX::getMapAtIndex(m_forward_segconns, cursor)->first.dir;
      break;
    case SEG_CONN_BK:
