...
std::vector<Point2f> parsePoints(std::istream &stream, char delimiter = '\t') {
  ...
  if (strings.size() < 2) {
    throw EntityParseException(
        "Badly formatted header (should contain x and y)");
  }
...
std::vector<IsovistDefinition> parseIsovists(std::istream &stream,
                                             char delimiter) {
  ...
  if (strings.size() < 2) {
    throw EntityParseException(
        "Badly formatted header (should contain x, y, can also have "
  ...
  if (xcol == -1 || ycol == -1) {
    throw EntityParseException(
        "Badly formatted header (should contain x and y, might also "
...
IsovistDefinition parseIsovist(const std::string &isovist) {
  ...
  if (parts.size() == 2) {
    return IsovistDefinition(std::atof(parts[0].c_str()),
    ...
    return IsovistDefinition(std::atof(parts[0].c_str()),
                             std::atof(parts[1].c_str()), angle, viewAngle);
  }
  std::stringstream message;
...
std::vector<std::pair<int, int>> parseRefPairs(std::istream &stream,
                                               char delimiter = '\t') {
  ...
  if (strings.size() < 2) {
    throw EntityParseException(
        "Badly formatted header (should contain reffrom and refto)");
  }
