...
void Isovist::makeit(BSPNode *root, const Point2f &p, const QtRegion &region,
  ...
  for (; curr != m_blocks.end(); ++curr) {
    if (!complete && !markedcentre && !parity &&
        curr->startangle == startangle) {
      // centre
      m_poly.push_back(p);
      // perimeter! occlusivity!
      markedcentre = true;
    }
    if (curr != m_blocks.begin() &&
        !approxeq(prev->endpoint, curr->startpoint, tolerance)) {
  ...
  if (m_blocks.size() && !approxeq(m_blocks.rbegin()->endpoint,
                                   m_blocks.begin()->startpoint, tolerance)) {
    m_poly.push_back(m_blocks.begin()->startpoint);
    // record perimeter information:
    double occluded =
        dist(m_blocks.rbegin()->endpoint, m_blocks.begin()->startpoint);
    m_perimeter += occluded;
    m_occluded_perimeter += occluded;
    // record the near *point* for use in agent analysis
    // (as the point will not move between isovists, so can record *which*
    ...
    if (occluded > 1.5) {
      if (dist(m_blocks.rbegin()->endpoint, m_centre) <
        ...
        m_occlusion_points.push_back(
            PointDist(m_blocks.rbegin()->endpoint, occluded));
      } else {
        m_occlusion_points.push_back(
            PointDist(m_blocks.begin()->startpoint, occluded));
      }
    }
  }
}
...
void Isovist::addBlock(const Line &li, int tag, double startangle,
                       double endangle) {
  ...
  while (!finished) {
    ...
    if (gap != m_gaps.end() && gap->startangle < endangle + 1e-9) {
      ...
      Point2f pa =
          intersection_point(li, Line(m_centre, m_centre + pointfromangle(a)));
      Point2f pb =
          intersection_point(li, Line(m_centre, m_centre + pointfromangle(b)));
