...
namespace depthmapX {
std::vector<PixelRefPair>
pixelateMergeLines(const std::vector<Line> &mergeLines, PointMap &currentMap) {
  std::vector<PixelRefPair> mergePixelPairs;

  std::vector<Line>::const_iterator iter = mergeLines.begin(),
                                    end = mergeLines.end();
  for (; iter != end; ++iter) {
...
void mergePixelPairs(const std::vector<PixelRefPair> &links,
                     PointMap &currentMap) {

  // check if any link pixel already exists on the map
  std::vector<PixelRefPair>::const_iterator iter = links.begin(),
                                            end = links.end();
  for (; iter != end; ++iter) {
    ...
    for (; prevIter != iter; ++prevIter) {
      ...
      if (link.a == prevLink.a || link.b == prevLink.b ||
          link.a == prevLink.b || link.b == prevLink.a) {
  ...
  std::for_each(links.begin(), links.end(),
                [&](const PixelRefPair &pair) -> void {
                  currentMap.mergePixels(pair.a, pair.b);
                });
}

void unmergePixelPairs(const std::vector<PixelRefPair> &links,
                       PointMap &currentMap) {
  ...
  std::for_each(links.begin(), links.end(),
                [&](const PixelRefPair &pair) -> void {
                  currentMap.unmergePixel(pair.a);
                });
}
...
std::vector<SimpleLine> getMergedPixelsAsLines(PointMap &currentMap) {
  ...
  std::vector<std::pair<PixelRef, PixelRef>>::const_iterator
      iter = mergedPixelPairs.begin(),
      end = mergedPixelPairs.end();
  for (; iter != end; ++iter) {
    std::pair<PixelRef, PixelRef> pixelPair = *iter;
    mergedPixelsAsLines.push_back(
        SimpleLine(currentMap.depixelate(pixelPair.first),
