...
bool PointMap::setGrid(double spacing, const Point2f &offset) {
  ...
  m_cols =
      (int)floor((xoffset + m_parentRegion->width()) / m_spacing + 0.5) + 1;
  m_rows =
      (int)floor((yoffset + m_parentRegion->height()) / m_spacing + 0.5) + 1;

  m_bottom_left = Point2f(m_parentRegion->bottom_left.x + m_offset.x,
                          m_parentRegion->bottom_left.y + m_offset.y);

  m_region = QtRegion(
      Point2f(m_bottom_left.x - m_spacing / 2.0,
              m_bottom_left.y - m_spacing / 2.0),
      Point2f(
          m_bottom_left.x + double(m_cols - 1) * m_spacing + m_spacing / 2.0,
      ...
      for (int j = s_bl.y; j <= s_tr.y; j++) {
        ...
        if (pnt.m_state & (Point::SELECTED | Point::FILLED)) {
          ...
          if (!pnt.m_merge.empty()) {
            ...
            depthmapX::findAndErase(m_merge_lines,
                                    PixelRefPair(PixelRef(i, j), p));
...
PixelRef PointMap::pixelate(const Point2f &p, bool constrain,
...
bool PointMap::blockLines(std::vector<Line> &lines) {
  ...
  for (size_t i = 0; i < m_cols; i++) {
    for (size_t j = 0; j < m_rows; j++) {
      ...
      std::vector<Line>::iterator iter = pt.m_lines.begin(),
                                  end = pt.m_lines.end();
      for (; iter != end;) {
...
bool PointMap::makePoints(const Point2f &seed, int fill_type,
  ...
  for (const auto &line : linesTouching) {
    if (intersect_line_no_touch(line,
                                Line(seed, getPoint(seedref).m_location))) {
      return false;
    }
  }
...
int PointMap::expand(const PixelRef p1, const PixelRef p2, PixelRefVector &list,
                     int filltype) {
...
void PointMap::outputMergeLines(std::ostream &stream, char delim) {
  stream << "x1" << delim << "y1" << delim << "x2" << delim << "y2"
         << std::endl;
  ...
  for (size_t i = 0; i < m_merge_lines.size(); i++) {
    ...
    stream << li.start().x << delim << li.start().y << delim << li.end().x
           << delim << li.end().y << std::endl;
  }
}
  ...
  for (size_t i = 0; i < m_cols; i++) {
    for (size_t j = 0; j < m_rows; j++) {
      ...
      if (pnt.filled() && pnt.m_node) {
        ...
        for (PixelRef &p : hood) {
          if (!(std::find(seenPix.begin(), seenPix.end(), p) !=
                seenPix.end()) &&
              getPoint(p).filled()) {
...
PafColor PointMap::getPointColor(PixelRef pixelRef) const {
  ...
  if (state & Point::HIGHLIGHT) {
    ...
    if (state & Point::FILLED) {
      if (m_processed) {
        return dXreimpl::getDisplayColor(
            AttributeKey(pixelRef),
            m_attributes->getRow(AttributeKey(pixelRef)), *m_attribHandle.get(),
            true);
...
bool PointMap::setCurSel(const std::vector<int> &selset, bool add) {
  ...
  for (size_t i = 0; i < selset.size(); i++) {
    ...
    if (includes(pix)) {
      m_points(static_cast<size_t>(pix.y), static_cast<size_t>(pix.x))
          .m_state |= Point::SELECTED;
        ...
        if (bounds.encloses(temp) && getPoint(temp).blocked()) { // Top Left
          ...
          if (bounds.encloses(temp) && getPoint(temp).blocked()) { // Left
            ...
            if (bounds.encloses(temp) &&
                getPoint(temp).blocked()) { // Bottom Left
              ba = true;
            } else {
              temp = temp.right();
  ...
  for (size_t j = 0; j < m_cols; j++) {
    for (size_t k = 0; k < m_rows; k++) {
      ...
      if (!m_hasIsovistAnalysis) {
        for (int b = 0; b < 32; b++) {
          if (m_points(k, j).m_node &&
              m_points(k, j).m_node->occdistance(b) > 0) {
    ...
    for (size_t k = 0; k < m_rows; k++) {
      ...
      pnt.m_state &= (Point::EMPTY | Point::FILLED | Point::MERGED |
                      Point::BLOCKED | Point::CONTEXTFILLED | Point::EDGE);
      ...
      if (!pnt.m_merge.empty()) {
        depthmapX::addIfNotExists(m_merge_lines,
                                  PixelRefPair(PixelRef(j, k), pnt.m_merge));
      }
    }
  }
...
bool PointMap::write(std::ostream &stream) {
  ...
  int sortedDisplayedAttribute =
      m_attributes->getColumnSortedIndex(m_displayed_attribute);
  stream.write((char *)&sortedDisplayedAttribute,
...
bool PointMap::sieve2(sparkSieve2 &sieve, std::vector<PixelRef> &addlist, int q,
                      int depth, PixelRef curs) {
  bool hasgaps = false;
  int firstind = 0;
  ...
  for (auto iter = sieve.m_gaps.begin(); iter != sieve.m_gaps.end(); ++iter) {
    ...
    for (int ind = (int)ceil(iter->start * (depth - 0.5) - 0.5);
      ...
      PixelRef here = PixelRef(curs.x + (q % 2 ? x : -x),
                               curs.y + (q <= 1 || q >= 6 ? y : -y));
...
bool PointMap::mergePixels(PixelRef a, PixelRef b) {
  ...
  if (a != b && getPoint(a).m_merge != b) {
    if (!getPoint(a).m_merge.empty()) {
      PixelRef c = getPoint(a).m_merge;
      auto it = std::find(m_merge_lines.begin(), m_merge_lines.end(),
                          PixelRefPair(a, c));
...
double PointMap::getLocationValue(const Point2f &point) {
  ...
  if (!getPoint(pix).filled()) {
    ...
    val =
        m_attributes->getRow(AttributeKey(pix)).getValue(m_displayed_attribute);
  }
