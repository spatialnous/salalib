...
void loadSalaProgram() {
  ...
  g_sala_math_ops.push_back(
      SalaFuncLabel(SalaObj::S_SUBTRACT, "-", "subtract"));
  ...
  g_sala_math_ops.push_back(
      SalaFuncLabel(SalaObj::S_MULTIPLY, "*", "multiply"));
  ...
  g_sala_math_ops.push_back(
      SalaFuncLabel(SalaObj::S_ASSIGN, "=", "assignment"));
  g_sala_math_ops.push_back(
      SalaFuncLabel(SalaObj::S_LIST_ACCESS, "[]",
  ...
  g_sala_comp_ops.push_back(
      SalaFuncLabel(SalaObj::S_GEQ, ">=", "greater than or equal to"));
  g_sala_comp_ops.push_back(
      SalaFuncLabel(SalaObj::S_LEQ, "<=", "less than or equal to"));
  g_sala_comp_ops.push_back(
      SalaFuncLabel(SalaObj::S_NEQ, "!=", "not equal to"));
  g_sala_comp_ops.push_back(SalaFuncLabel(SalaObj::S_EQ, "==", "equal to"));
  g_sala_comp_ops.push_back(
      SalaFuncLabel(SalaObj::S_IS, "is", "is the same object as"));

  // logical ops
  g_sala_logical_ops.push_back(
      SalaFuncLabel(SalaObj::S_NOT, "not", "logical not"));
  g_sala_logical_ops.push_back(
      SalaFuncLabel(SalaObj::S_NOT, "!", "logical not"));
  g_sala_logical_ops.push_back(
      SalaFuncLabel(SalaObj::S_AND, "and", "logical and"));
  g_sala_logical_ops.push_back(
      SalaFuncLabel(SalaObj::S_AND, "&&", "logical and"));
  g_sala_logical_ops.push_back(
      SalaFuncLabel(SalaObj::S_OR, "or", "logical or"));
  g_sala_logical_ops.push_back(
      SalaFuncLabel(SalaObj::S_OR, "||", "logical or"));

  // global functions
  g_sala_global_funcs.push_back(
      SalaFuncLabel(SalaObj::S_SQRT, "sqrt", "square root"));
  g_sala_global_funcs.push_back(
      SalaFuncLabel(SalaObj::S_LOG, "log", "log base 10"));
  g_sala_global_funcs.push_back(
      SalaFuncLabel(SalaObj::S_LN, "ln", "natural logarithm"));
  g_sala_global_funcs.push_back(
  ...
  g_sala_global_funcs.push_back(
      SalaFuncLabel(SalaObj::S_TAN, "tan", "tangent"));
  g_sala_global_funcs.push_back(
      SalaFuncLabel(SalaObj::S_ASIN, "asin", "inverse sine"));
  g_sala_global_funcs.push_back(
      SalaFuncLabel(SalaObj::S_ACOS, "acos", "inverse cosine"));
  g_sala_global_funcs.push_back(
      SalaFuncLabel(SalaObj::S_ATAN, "atan", "inverse tangent"));
  g_sala_global_funcs.push_back(
      SalaFuncLabel(SalaObj::S_LEN, "len", "array or string length"));
  g_sala_global_funcs.push_back(
      SalaFuncLabel(SalaObj::S_RANGE, "range", "set of integers"));

  // member functions
  g_sala_member_funcs.push_back(SalaMemberFuncLabel(
  ...
  g_sala_member_funcs.push_back(
      SalaMemberFuncLabel(SalaObj::S_GRAPHOBJ, SalaObj::S_FSETVALUE, "setvalue",
                          "set attribute value"));
  g_sala_member_funcs.push_back(SalaMemberFuncLabel(
  ...
  g_sala_member_funcs.push_back(
      SalaMemberFuncLabel(SalaObj::S_GRAPHOBJ, SalaObj::S_FCONNECTIONS,
    ...
    if (!program.eof()) {
      ...
      if (thiscommand.m_command == SalaCommand::SC_FOR ||
        ...
        if (parent->m_children.size() < 2) {
          m_error_stack.push_back(
              SalaError("'Else' must be preceded by an 'if','for' or 'while'",
                        thiscommand.m_line));
          return false;
        }
        int command =
            parent->m_children[parent->m_children.size() - 2].m_command;
        if (command != SalaCommand::SC_IF && command != SalaCommand::SC_ELIF &&
            command != SalaCommand::SC_FOR &&
            command != SalaCommand::SC_WHILE) {
          m_error_stack.push_back(
              SalaError("'Else' must be preceded by an 'if','for' or 'while'",
                        thiscommand.m_line));
          return false;
        ...
        if (parent->m_children.size() < 2) {
          m_error_stack.push_back(
              SalaError("'Elif' must be preceded by an 'if' condition",
                        thiscommand.m_line));
          return false;
        }
        int command =
...
bool SalaProgram::runselect(std::vector<int> &selsetout,
                            const std::set<int> &selsetin) {
  AttributeTable *table = m_thisobj.getTable();
...
std::string SalaProgram::getLastErrorMessage() const {
  ...
  if (error.lineno == -1) {
    ...
    return error.message + " on line " +
           dXstring::formatString(error.lineno + 1, "%d");
  }
}

////////////////////////////////////////////////////////////////////////////

SalaCommand::SalaCommand(SalaProgram *program, SalaCommand *parent, int indent,
                         Command command) {
  m_program = program;
  m_parent = parent;
  m_indent = indent;
...
int SalaCommand::parse(std::istream &program, int line) {
  ...
  while (!endloop && !program.eof()) {
    ...
    case '-':
      ...
      if (last == SP_FUNCTION || last == SP_COMMAND) {
        pushFunc(alpha == '+' ? SalaObj(SalaObj::S_PLUS)
                              : SalaObj(SalaObj::S_MINUS));
      } else {
        pushFunc(alpha == '+' ? SalaObj(SalaObj::S_ADD)
                              : SalaObj(SalaObj::S_SUBTRACT));
      }
      last = SP_FUNCTION;
      break;
    case '=':
    ...
    case '(':
      ...
      if (last == SP_DATA) {
        ...
        throw SalaError(m_last_string + " is not a known function name",
                        m_line);
        // (in the future, we may well want to transfer an object hashed
        // function name to the func stack instead)
      } else if (last == SP_NUMBER) {
        throw SalaError("Cannot treat a number as if it were a function",
                        m_line);
      }
    ...
    case '[':
      ...
      // value or empty list depending on context
      {
        ...
        if (beta == ']') {
          alpha = program.get();
          if (last == SP_DATA) {
            throw SalaError("Accessor operator ('[]') requires a parameter",
                            m_line);
          } else {
        ...
        while (!program.eof() && alpha != '\n') {
          ...
          if (!commentfound) {
            if (alpha == '#') {
              ...
              throw SalaError(
                  "'For', 'if', 'else', etc cannot have execution part "
    ...
    case '\t':
      throw SalaError(
          "Tab character found: please use only spaces to indent lines",
          m_line);
      break;
    default:
      if (strchr("<>=!", cache)) {
      ...
      if (alpha != EOF &&
        ...
        if (!isalphanum_(alpha) && alpha != '&' &&
          ...
          throw SalaError(
              "Unrecognised symbol ('" + std::string(1, alpha) + "')", m_line);
        }
        buffer.add(alpha);
      }
      break;
  ...
  if (m_eval_stack.size() == 0 &&
      m_command != SC_ELSE) { // note, else is by definition empty
    throw SalaError("Partial or missing command", m_line);
  }
  return line;
}

int SalaCommand::decode(
    std::string string) // string copied as makelower applied
{
  ...
  if (m_command == SC_NONE) {
    if (string == "return") {
      ...
      m_command =
          SC_FOR; // n.b. will still need a variable name and "in": for x in ...
      retvar = SP_COMMAND;
  ...
  // numeric constant
  if (isdigit(string[0]) ||
      (string.length() > 1 && string[0] == '.' && isdigit(string[1]))) {
  ...
  if (retvar == SP_NONE) {
    ...
    if (n != std::string::npos) {
      ...
      if (decode_member(string.substr(n + 1), false) == SP_NONE) {
        throw SalaError("There is no known member function called " +
                            string.substr(n + 1),
                        m_line);
      }
      retvar = SP_FUNCTION;
  ...
  if (retvar == SP_NONE) {
    ...
    throw SalaError("There is no known function or variable called " + string,
                    m_line);
  }
...
int SalaCommand::decode_member(const std::string &string, bool apply_to_this) {
  ...
  for (size_t i = 0; i < g_sala_member_funcs.size(); i++) {
    ...
    if (!apply_to_this ||
        (m_program->m_thisobj.type & g_sala_member_funcs[i].type) != 0) {
...
void SalaCommand::pushFunc(const SalaObj &func) {
  ...
  if (func.type & SalaObj::S_BRACKET) {
    if (func.type == SalaObj::S_CLOSE_BRACKET) {
      while (m_func_stack.size() &&
             m_func_stack.back().type != SalaObj::S_OPEN_BRACKET) {
        m_eval_stack.push_back(m_func_stack.back());
        m_func_stack.pop_back();
      }
      ...
      m_func_stack.push_back(func);
             ...
             func.precedence() >
                 m_func_stack.back().precedence()) { // original: >
    m_func_stack.push_back(func);
  } else {
    while (m_func_stack.size() &&
           func.precedence() <=
    ...
    if (list.type == SalaObj::S_LIST) {
      ...
      if (len != 0) {
        for (int i = 0; i < len; i++) {
          ...
          m_program->m_var_stack[m_for_iter.data.var] =
              list.data.list.list->at(i);
...
SalaObj SalaCommand::evaluate(int &pointer, SalaObj *&p_obj) {
  ...
  if (data.type == SalaObj::S_FUNCTION) {
      ...
      try {
        ...
        case SalaObj::S_ASSIGN:
          ...
          if (p_obj != nullptr) {
            *p_obj = data;
            ...
            throw SalaError("Cannot assign to constant, function or none",
                            m_line);
          }
        ...
        for (size_t i = 0; i < g_sala_math_ops.size(); i++) {
          if (g_sala_math_ops[i].func == func) {
            e.message =
                "In '" + g_sala_math_ops[i].name + "' operator: " + e.message;
            break;
          }
        }
        e.lineno = m_line;
      ...
      try {
        ...
        for (size_t i = 0; i < g_sala_logical_ops.size(); i++) {
          if (g_sala_logical_ops[i].func == func) {
            e.message = "In '" + g_sala_logical_ops[i].name +
                        "' operator: " + e.message;
            break;
          }
        }
        for (size_t j = 0; j < g_sala_comp_ops.size(); j++) {
          if (g_sala_comp_ops[j].func == func) {
            e.message =
                "In '" + g_sala_comp_ops[j].name + "' operator: " + e.message;
            break;
          }
        }
        e.lineno = m_line;
      ...
      try {
        ...
        for (size_t i = 0; i < g_sala_global_funcs.size(); i++) {
          if (g_sala_global_funcs[i].func == func) {
            e.message = "In '" + g_sala_global_funcs[i].name +
                        "' function: " + e.message;
            break;
          }
        }
      ...
      try {
        ...
        case SalaObj::S_TUPLE:
          ...
          case SalaObj::S_FCLEAR:
            ...
            break;
          default:
            throw SalaError("Not a member function of " + obj.getTypeStr(),
                            m_line);
          }
          break;
        ...
        case SalaObj::S_POINTMAPOBJ:
          ...
          case SalaObj::S_FSETVALUE: {
            ...
            if (str != "Ref Number") {
              ...
              throw SalaError("The reference number can not be changed",
                              m_line);
            }
        ...
        for (size_t i = 0; i < g_sala_member_funcs.size(); i++) {
          if (g_sala_member_funcs[i].func == func) {
            ...
            e.message = "In " + type.getTypeStr() + " '" +
                        g_sala_member_funcs[i].name +
                        "' function: " + e.message;
            break;
          }
        }
    ...
    data = SalaObj((data.type == SalaObj::S_CONST_LIST) ? SalaObj::S_LIST
                                                        : SalaObj::S_TUPLE,
                   x);
    for (--x; x >= 0; x--) {
...
SalaObj SalaCommand::connections(SalaObj graphobj, SalaObj param) {
  ...
  if ((graphobj.type & SalaObj::S_MAP) == SalaObj::S_POINTMAP) {
    // point map version
    Node &node =
        graphobj.data.graph.map.point->getPoint(graphobj.data.graph.node)
            .getNode();
    if (param.type == SalaObj::S_NONE) {
    ...
    int idx =
        std::distance(graphobj.data.graph.map.shape->getAllShapes().begin(),
                      graphobj.data.graph.map.shape->getAllShapes().find(
                          graphobj.data.graph.node));
    const Connector &connector =
        graphobj.data.graph.map.shape->getConnections()[idx];
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (connectedIndex == -1) {
        ...
        graphobj.data.graph.node =
            graphobj.data.graph.map.shape->getShapeRefFromIndex(connectedIndex)
                ->first;
      }
      list.data.list.list->at(i) = graphobj;
      cursor++;
