  ...
  if (a.x == b.x) {
    ...
    while (a.x < b.x) {
      ...
      pixel_list.push_back(
          PixelRef(a.x, parity * a.y)); // Lines always go left to right
    }
  } else {

    double hw_ratio =
        l.height() / l.width(); // Working all of these out leaves less scope
    ...
    while (a.x < b.x || a.y < b.y) {
      PixelRef e;
      e.y =
          parity * int(double(scaledrows) *
                       (x0_const + parity * hw_ratio *
                                       (double(a.x + 1) / double(scaledcols))));
  ...
  if (dir == XAXIS) {
    ...
    for (int i = first; i <= last; i++) {
      int j1 = (int)floor((first == i ? l.ax() : double(i)) * grad + constant -
                          l.sign() * tolerance);
      int j2 = (int)floor((last == i ? l.bx() : double(i + 1)) * grad +
    ...
    for (int i = first; i <= last; i++) {
      int j1 = (int)floor((first == i ? l.bottom_left.y : double(i)) * grad +
                          constant - l.sign() * tolerance);
      int j2 = (int)floor((last == i ? l.top_right.y : double(i + 1)) * grad +
                          constant + l.sign() * tolerance);
...
SpacePixel::SpacePixel(const SpacePixel &spacepixel)
    : m_pixel_lines(spacepixel.m_pixel_lines.rows(),
                    spacepixel.m_pixel_lines.columns()) {
  ...
  */
  ...
  for (int i = bl.x; i <= tr.x; i++) {
    for (int j = bl.y; j <= tr.y; j++) {
      auto &pixel_lines =
          m_pixel_lines(static_cast<size_t>(j), static_cast<size_t>(i));
      for (int pixel_line : pixel_lines) {
        m_display_lines[size_t(
            depthmapX::findIndexFromKey(m_lines, pixel_line))] = 1;
      }
    }
  }
}
...
void SpacePixel::initLines(int size, const Point2f &min, const Point2f &max,
                           double density) {
  m_display_lines.clear();
  m_lines.clear();
  m_ref = -1;
  ...
  m_pixel_lines = depthmapX::RowMatrix<std::vector<int>>(
      static_cast<size_t>(m_rows), static_cast<size_t>(m_cols));
}
...
void SpacePixel::reinitLines(double density) {
  ...
  for (const auto &line : m_lines) {
    ...
    for (size_t j = 0; j < list.size(); j++) {
      ...
      m_pixel_lines(static_cast<size_t>(list[j].y),
                    static_cast<size_t>(list[j].x))
...
void SpacePixel::addLine(const Line &line) {
  ...
  for (size_t i = 0; i < list.size(); i++) {
    // note: m_pixel_lines will be reordered by sortPixelLines
    m_pixel_lines(static_cast<size_t>(list[i].y),
...
int SpacePixel::addLineDynamic(const Line &line) {
  ...
  for (size_t i = 0; i < list.size(); i++) {
    ...
    if (list[i].x >= 0 && list[i].y >= 0 &&
        static_cast<size_t>(list[i].x) < m_cols &&
...
void SpacePixel::sortPixelLines() {
  for (size_t i = 0; i < static_cast<size_t>(m_cols); i++) {
    for (size_t j = 0; j < static_cast<size_t>(m_rows); j++) {
      ...
      for (auto rev_iter = pixel_lines.rbegin(); rev_iter != pixel_lines.rend();
           ++rev_iter) {
...
bool SpacePixel::intersect(const Line &l, double tolerance) {
  ...
  for (size_t i = 0; i < list.size(); i++) {
    auto &pixel_lines = m_pixel_lines(static_cast<size_t>(list[i].y),
                                      static_cast<size_t>(list[i].x));
        ...
        if (intersect_region(linetest.line, l)) {
          if (intersect_line(linetest.line, l, tolerance)) {
            if (linetest.line.start() != l.start() &&
                linetest.line.start() != l.end() &&
                linetest.line.end() != l.start() &&
...
void SpacePixel::cutLine(Line &l, short dir) {
  ...
  for (size_t i = 0; i < vec.size() && !found; i++) {
    ...
    auto &pixel_lines =
        m_pixel_lines(static_cast<size_t>(pix.y), static_cast<size_t>(pix.x));
    for (int lineref : pixel_lines) {
      // try {
      LineTest &linetest = m_lines.find(lineref)->second;
      if (linetest.test != m_test) {
        if (intersect_region(linetest.line, l,
          ...
          switch (intersect_line_distinguish(
              linetest.line, l, tolerance * linetest.line.length())) {
          case 0:
            break;
          case 2: {
          ...
          case 1:
            if (truestart != linetest.line.start() &&
              ...
              if (!touching_lines.size()) {
                ...
                for (size_t k = 0; k < touching_lines.size() && pair == -1;
                     k++) {
                  ...
                  if (pair != -1) {
                    ...
                    if (sgn(oa) != sgn(ob) ||
                      ...
                      // crossed
                      if (fabs(oa) >
                          tolerance * linetest.line
                                          .length()) { // checks not parallel...
                        loc.insert(l.intersection_point(linetest.line, axis));
                      } else if (fabs(ob) >
                                 tolerance * linetest.line.length()) {
                        loc.insert(
                            l.intersection_point(touching_lines[pair], axis));
                      } else {
...
bool SpacePixel::read(std::istream &stream) {
  ...
  for (const auto &line : m_lines) {
    ...
    for (size_t m = 0; m < list.size(); m++) {
      ...
      m_pixel_lines(static_cast<size_t>(list[m].y),
                    static_cast<size_t>(list[m].x))
          .push_back(n);
    }
  }
