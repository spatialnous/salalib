...
bool sparkSieve2::testblock(const Point2f &point,
                            const std::vector<Line> &lines, double tolerance) {
  ...
  for (const auto &line : lines) {
    ...
    if (intersect_region(l, line, tolerance) &&
        intersect_line(l, line, tolerance)) {
      return true;
    }
  }
...
void sparkSieve2::collectgarbage() {
  ...
  for (; blockIter != m_blocks.end() && iter != m_gaps.end();) {
    ...
    if (iter->end <=
        iter->start + 1e-10) { // 1e-10 required for floating point error
