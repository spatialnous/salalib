...
void TidyLines::tidy(std::vector<Line> &lines, const QtRegion &region) {
  ...
  lines.erase(std::remove_if(lines.begin(), lines.end(),
                             [maxdim](const Line &line) {
                               return line.length() < maxdim * TOLERANCE_B;
                             }),
              lines.end());
  ...
  for (size_t i = 0; i < lines.size(); i++) {
    ...
    for (size_t a = 0; a < list.size(); a++) {
      auto pixel_lines = m_pixel_lines(static_cast<size_t>(list[a].y),
                                       static_cast<size_t>(list[a].x));
        ...
        if (m_lines[j].test != m_test && j > (int)i &&
          ...
          if (axis_i == axis_j &&
              fabs(lines[i].grad(axis_reverse) - lines[j].grad(axis_reverse)) <
                  TOLERANCE_A &&
              fabs(lines[i].constant(axis_reverse) -
                   lines[j].constant(axis_reverse)) < (TOLERANCE_B * maxdim)) {
  ...
  removelist.erase(std::unique(removelist.begin(), removelist.end()),
                   removelist.end());
...
void TidyLines::quicktidy(std::map<int, std::pair<Line, int>> &lines,
  ...
  for (const auto &line : lines) {
    ...
    auto &pixel_lines = m_pixel_lines(static_cast<size_t>(start.y),
                                      static_cast<size_t>(start.x));
    for (int k : pixel_lines) {
      if (k > int(i) && approxeq(m_lines[i].line.start(),
                                 m_lines[k].line.start(), tolerance)) {
